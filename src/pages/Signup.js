...
import { Message, Form, Button } from 'semantic-ui-react'; // 導入 Semantic UI React 的 Form 元件
...
export default function Signup({
  ...
  const onSubmit = async(e) =>{
    setErrorMessage('');
    const result = await handleAuthAction('register', email, password)
    console.log(result);
    if (result === true) {
      navigate('/')
              ...
              <Heading>{headingText}</Heading>
                <FormContainer>
                  <Form onSubmit={onSubmit}>
                      ...
                      />
                      {errorMessage && <Message negative color='red'>{errorMessage}</Message>}
